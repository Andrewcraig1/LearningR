...
# Load our data which includes txt:
...
# Convert and Save this table as a data frame:
...
# where the first argument is our variable, WeekdayCounts$Var1, the second argument is ordered = TRUE.This says that we want an ordered factor.
# And the third argument, which is levels, should be equal to a vector of the days of the week in the order we want them to be in. We'll use the c function to do this.
...
# Change our x and y labels and add in argument "linetype":
...
#  To make the line lighter in colour change the alpha parameter to 0.3 
# by replacing "linetype=2" with "alpha=0.3" in the plot command
...
# Convert factor variable into a numeric variable.
# i.e. Convert the second variable, Var2, to numbers and call it Hour:
...
# Fix order of days so they show up in chronological order
DayHourCounts$Var1<-factor(DayHourCounts$Var1,ordered=TRUE, levels = c("Monday","Tuesday","Wednesday","Thursday","Friday", "Saturday","Sunday"))

# Create out heat map. argument "tile" is the function used to make a heat map.
ggplot(DayHourCounts, aes(x = Hour, y = Var1)) + geom_tile(aes(fill=Freq)) +
        scale_fill_gradient(name="Total MV Thefts", low="white",high="red") + 
        theme(axis.title.y = element_blank())

# note argument "axis.title.y = element_blank()" means no y axis label.
...
# QQ remove Freq of 0 from heat map
LatLonCounts2<-subset(LatLonCounts, Freq>0)
str(LatLonCounts2)

# QQConvert our Longitude and Latitude variable to numbers:
...
# Tile heat map-  geom_tile geometry
...
# Plot the map using "geom_polygon" argument. fill= colour of polygon, color=colour of the line:
ggplot(statesMap, aes(x = long, y = lat, group = group)) + geom_polygon(fill = "white", color = "black") 

# Create a new variable called region with the lowercase names TO MATCH the statesMap:
murders$region <- tolower(murders$State)

# JOIN the statesMap data and the murders data into one dataframe with the "merge" function.
# The by="region" argument is the common identifier that mergers the rows correctly.
...
# Redo the plot, removing any states with murder rates above 10, using argument "limits = c(0,10)"
